...
#define RPC_METHOD_PLAY "play"
#define RPC_METHOD_PAUSE "pause"
#define RPC_METHOD_CLOSE "close"
#define RPC_METHOD_RELOAD "reload"
...
static void peer_signaling_on_pub_event(const char* msg, size_t size) {
  ...
  do {
	...
	} else if (strcmp(item->valuestring, RPC_METHOD_PLAY) == 0) {
      peer_connection_media_play(g_ps.pc);
	  LOGI("receive play signal\n");
	} else if (strcmp(item->valuestring, RPC_METHOD_PAUSE) == 0) {
      peer_connection_media_pause(g_ps.pc);
	  LOGI("receive play pause\n");
	} else if ((strcmp(item->valuestring, RPC_METHOD_CLOSE) == 0) ||
		(strcmp(item->valuestring, RPC_METHOD_RELOAD) == 0)) {
...
static int peer_signaling_mqtt_connect(const char *hostname, int port)
{
  ...
  if (status != MQTTSuccess) {
	  LOGE("MQTT_Init error\n");
  }
  status = MQTT_InitStatefulQoS(&g_ps.mqtt_ctx,
		pOutgoingPublishRecords,
		OUTGOING_PUBLISH_RECORD_LEN,
		pIncomingPublishRecords,
		INCOMING_PUBLISH_RECORD_LEN);
  if (status != MQTTSuccess) {
	  LOGE("MQTT_InitStatefulQoS error\n");
  }

  if (status != MQTTSuccess) return EXIT_FAILURE;
...
int peer_signaling_mqtt_reconnect()
{
/* If connect MQTT server failed, will reconnect */
  int reconnect_attempt = 5;
  int i = 0;

  LOGW("Connect MQTT server failed , need reconnect attempts");
  while (i++ < reconnect_attempt) {
    LOGW("MQTT reconnect try times = %d", i);
    if (peer_signaling_mqtt_connect(g_ps.mqtt_host, g_ps.mqtt_port) == 0) {
      LOGI("MQTT reconnect success");
      break;
    } else {
      LOGW("MQTT reconnect failed , Please check network");
      return -1;
    }
    sleep(1);
  }
...
int peer_signaling_join_channel()
{
  ...
  if (peer_signaling_mqtt_connect(g_ps.mqtt_host, g_ps.mqtt_port) < 0) {
    LOGW("Connect MQTT server failed");
      if (peer_signaling_mqtt_reconnect() < 0) {
        return -1;
      }
  }
...
int peer_signaling_loop() {
  ...
  if (g_ps.mqtt_port > 0) {
    eMqttStatus = MQTT_ProcessLoop(&g_ps.mqtt_ctx);
      if (!(eMqttStatus == MQTTSuccess || eMqttStatus == MQTTNeedMoreBytes)) {
        LOGW("MQTT_ProcessLoop error, eMqttStatus = %s\n", MQTT_Status_strerror(eMqttStatus));
        if (peer_signaling_mqtt_reconnect() < 0) {
          LOGE("peer_signaling_mqtt_reconnect error");
        }
      }
  }
  return 0;
