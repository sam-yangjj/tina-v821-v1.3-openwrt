...
/* 设置固件监测主控的功能：固件运行过程中监测主控发送的cmd或data
   WiFi驱动默认每2秒至少向固件发送一个cmd，保持接口处于活动状态。
   参数 timeout：超时时间，单位为秒。超过timeout时间未接收到主控任何数据，固件则认为与主控的接口通信出现异常，固件自动复位。
*/
int hgic_iwpriv_set_host_monitor(char *ifname, int timeout)
{
    return hgic_iwpriv_set_int(ifname, "host_mntr", timeout);
}

/* 设置固件的接口DMA聚合功能
   参数 scatter_size ：DMA聚合大小，0表示关闭聚合。
*/
...
/*设置固件休眠漫游功能：休眠期间监测AP的信号强度，达到设置的阈值后自动唤醒，并设置唤醒原因为25.  
  参数 enable：开启/关闭 休眠漫游功能
  参数 rssi: 设置监测AP的信号强度阈值，低于该阈值则自动唤醒。
*/
...
/*设置固件EDCA参数的最大值。 该参数会影响4路EDCA参数的回退时间，不会超过该参数设置的最大值
  参数 param: EDCA参数，每个值的含义表示最大值。
*/
...
/*用于限制lmac所有EDCA参数的最小值（主要用于限制txop limit的最小值）。默认值为无限制。
  参数 param: txq_param为EDCA参数结构体，只不过里面的所有参数的含义都变成了EDCA允许配置的最小值。
              这个函数会同时作用与4个EDCA。EDCA限制的txop limit最小值越大，抢到信道后连续占用信道的时间就越长。
*/
int hgic_iwpriv_set_edca_min(char *ifname, struct hgic_txq_param *param)
{
    return hgic_iwpriv_set_bytes(ifname, "edca_min", (char *)param, sizeof(struct hgic_txq_param));
}

/*设置固件EDCA slot time参数。通过修改该参数可以控制CCA回退时间长短。
  参数：slot_time，单位是 us。固件默认slot time是9us 
*/
