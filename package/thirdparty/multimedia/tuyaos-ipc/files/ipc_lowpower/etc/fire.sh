...
partition_size=$(cat /proc/partitions | grep "mtdblock7" | tr -s ' ' | cut -d' ' -f4)
...
if [ "$firmware_size" -gt "$partition_size" ]; then
    echo "error, exit! rootfs.squashfs bigger than /dev/mtd7 "
    exit 1  # 停止升级过程
fi
...
flashcp -v /tmp/rootfs.squashfs /dev/mtd7
