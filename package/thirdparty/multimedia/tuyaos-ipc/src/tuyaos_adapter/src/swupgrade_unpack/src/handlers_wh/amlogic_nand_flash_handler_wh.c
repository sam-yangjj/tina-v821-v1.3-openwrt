...
int install_amlogic_nand_flash_image_wh(char *dev, ota_image *pimg, int offs, char *data, int length)
{
	int ret;
	...
	ret = nand_erase_rk(dev, offs);
	if (ret < 0) {
		ERROR("erase %s failed.\n", dev);
		return -1;
	}

	ret = nand_write_rk(dev, offs, pimg);
	if (ret < 0) {
