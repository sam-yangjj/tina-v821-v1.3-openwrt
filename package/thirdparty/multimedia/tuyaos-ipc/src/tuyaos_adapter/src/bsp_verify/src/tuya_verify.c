...
#include "tuya_verify_pubkey.h"
#if defined(TUYA_BSP_VERIFY_ONBOARD)
#include "bcmnvram.h"
#endif
...
#define TUYAOS_OTA_PUBKEY_LEN	(128)
#define TUYAOS_OTA_PUBKEY_NAME	("persist.ota.pubkey")
...
u8 *public_key_imported = NULL;

int tuya_set_public_key(unsigned char key[64])
{
    int i;

	if (key != NULL) {
		if (public_key_imported == NULL) {
			public_key_imported = (u8*)malloc(64);
			if (public_key_imported == NULL)
				return -1;
		}
		for (i = 0; i < 64; i++)
			public_key_imported[i] = key[i];
	} else if (public_key_imported != NULL) {
		free(public_key_imported);
		public_key_imported = NULL;
	}
...
#if defined(TUYA_BSP_VERIFY_ONBOARD)
static u8 *tuya_verify_get_pubkey_4nvram(void)
{
	int ret;
	char str[256];
	unsigned long data;
	...
	ret = nvram_get(TUYAOS_OTA_PUBKEY_NAME, str);
	if (ret<0)
	{
		printf("%s %d nvram get fail!\n", __func__, __LINE__);
	        return NULL;
	}

	str[TUYAOS_OTA_PUBKEY_LEN]='\0';
	printf("nvram get %s is %s\n", TUYAOS_OTA_PUBKEY_NAME, str);
	...
	for(i=0;i<64;i++)
	{
		strncpy((char *)buftmp, p1, 2);
		data = strtoul((char *)buftmp, NULL, 16);
		pubkey[i] = data;
		p1+=2;
	}
...
static void tuya_verify_put_pubkey(u8 *pubkey)
{
    if(pubkey)
        free(pubkey);
}
		...
		unsigned char* sign)
{
	...
	ret = bsp_verify_sha256(buf, buf_len, hash);
	if (ret) {
...
#if defined(TUYA_BSP_VERIFY_ONBOARD)
	pub_key_from_pem = tuya_verify_get_pubkey_4nvram();
	if(!pub_key_from_pem)
	{
		printf("get pubkey form nvram fail\n");
		return -1;
	}
#else
	pub_key_from_pem = public_key_new;
#endif

	pub_key = public_key_imported?public_key_imported:pub_key_from_pem;
	ret = ecdsa_verify(ECC_CURVE_SECP256K1, (u64*)pub_key, (u64*)hash, (u64*)sign);
	if (ret)
		PRERR("%s ecdsa_verify failed ret:%d\n", __func__, ret);

#if defined(TUYA_BSP_VERIFY_ONBOARD)
	tuya_verify_put_pubkey(pub_key_from_pem);
#endif
	return ret;
}
