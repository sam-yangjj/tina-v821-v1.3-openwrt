...
static void* v4l2_capture_thread(void* arg) {
    ...
    while (priv->running) {
        ...
        update_fps_v4l2();
        //if(frame_queue_push(&priv->queue, frame))

        if(frame_queue_push_timeout(&priv->queue, frame, 30))
        {
            static int num = 0;
        ...
        #if 0
        for (int p = 0; p < priv->num_planes; p++) {
            buf.m.planes[p].m.fd = priv->dma_buf_info.dma_buf_fds[buf.index][p];
            //planes[p].length = priv->dma_buf_info.plane_lengths[i][p];
        }
...
FrameBuffer* v4l2_camera_pull_frame(Plugin* plugin) {
    ...
    return frame_queue_pop_timeout(&priv->queue, 100);
}
