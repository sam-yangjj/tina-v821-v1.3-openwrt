...
#if 0
	if (j > 10) {
		isp_return_tdmbuffer(status->dev_id, status);
		return;
	}

	memset(&data, 0, sizeof data);
	data.buf = malloc(status->buf_size);
	if (data.buf == NULL)  {
		ISP_ERR("malloc size is %d err\n", status->buf_size);
		isp_return_tdmbuffer(status->dev_id, status);
		return;
	}
	data.buf_size = status->buf_size;
	isp_get_tdm_data(status->dev_id, &data);

	char fdstr[50];
	FILE *file_fd = NULL;
	sprintf(fdstr, "%s/pic%d.bin", path_name, j);
	file_fd = fopen(fdstr, "w");
	fwrite(data.buf + status->head_len + status->fill_len, data.buf_size - status->head_len - status->fill_len, 1, file_fd);
	fclose(file_fd);
	free(data.buf);
	j++;
#endif
...
static void *loop_cap(void *pArg)
{
	...
	//isp_register_tdmbuffer_done_callback(privCap->isp_id, &isp_tdm_buffer_process);
...
int main_test(int ch_num, int width, int height, int out_fmt)
{
	...
	for(i = 0; i < ch_num; i++) {
		...
		if (privCap[i].vfmt.large_dma_merge_en) {
			...
			isp_set_stitch_mode(privCap[i].isp_id, STITCH_2IN1_LINNER);
