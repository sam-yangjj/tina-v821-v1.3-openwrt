...
define TinaHooks/CopyFromDefconfig
	echo copy from $(TINA_TARGET_DEFCONFIG)$(OTA)
	[ -f "$(TINA_TARGET_DEFCONFIG)$(OTA)" ] && \
		cp $(TINA_TARGET_DEFCONFIG)$(OTA) .config || \
		echo "Can't find defconfig"
endef

define TinaHooks/CopyToDefconfig
	echo copy to $(TINA_TARGET_DEFCONFIG)$(OTA)
	[ -f "$(TINA_TARGET_DEFCONFIG)$(OTA)" ] && \
		cp .config $(TINA_TARGET_DEFCONFIG)$(OTA) || \
		echo "Can't find defconfig"
endef
