...
REALPATH_BUILD_LOG_DIR=$(if $(BUILD_LOG),$(realpath $(BUILD_LOG_DIR)))
      ...
      $(foreach btype,$(buildtypes-$(bd)),
        $(call warn_eval,$(1)/$(bd),t,T,$(1)/$(bd)/$(btype)/$(target): $(if $(NO_DEPS)$(QUILT),,$($(1)/$(bd)/$(btype)/$(target)) $(call $(1)//$(btype)/$(target),$(1)/$(bd)/$(btype))))
		  $(call log_make,$(1)/$(bd),$(target),$(btype),$(filter-out __default,$(variant))) \
			|| $(call ERROR,$(2),   ERROR: $(1)/$(bd) [$(btype)] failed to build.$(if $(BUILD_LOG)," "    More error defails please refer to: $(REALPATH_BUILD_LOG_DIR)/$(1)/$(bd)/$(filter-out __default,$(variant))/$(if $(btype),$(bytpe)-)$(target).txt),$(findstring $(bd),$($(1)/builddirs-ignore-$(btype)-$(target))))
      ...
      $(call warn_eval,$(1)/$(bd),t,T,$(1)/$(bd)/$(target): $(if $(NO_DEPS)$(QUILT),,$($(1)/$(bd)/$(target)) $(call $(1)//$(target),$(1)/$(bd))))
        $(foreach variant,$(if $(BUILD_VARIANT),$(BUILD_VARIANT),$(if $(strip $($(1)/$(bd)/variants)),$($(1)/$(bd)/variants),$(if $($(1)/$(bd)/default-variant),$($(1)/$(bd)/default-variant),__default))),
			...
			$(call log_make,$(1)/$(bd),$(target),,$(filter-out __default,$(variant))) \
				|| $(call ERROR,$(1),   ERROR: $(1)/$(bd) failed to build$(if $(filter-out __default,$(variant)), (build variant: $(variant))).$(if $(BUILD_LOG)," "    More error defails please refer to: $(REALPATH_BUILD_LOG_DIR)/$(1)/$(bd)/$(filter-out __default,$(variant))/$(target).txt),$(findstring $(bd),$($(1)/builddirs-ignore-$(target))))
	)
