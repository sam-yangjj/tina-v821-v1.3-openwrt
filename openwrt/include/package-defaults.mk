...
PKG_FASTBOOT_DEPENDS := +USE_FASTBOOT:kmod-fastboot-mod

ifneq ($(PKG_NAME),toolchain)
  ifneq ($(CONFIG_USE_FASTBOOT),y)
    PKG_FIXUP_DEPENDS = $(if $(filter kmod-%,$(1)),$(2),$(PKG_DEFAULT_DEPENDS) $(filter-out $(PKG_DEFAULT_DEPENDS),$(2)))
    TARGET_LDFLAGS += $(if $(filter kmod-%,$(1)),,$(if $(CONFIG_USE_GLIBC),-lrt -lpthread))
  else
    PKG_FIXUP_DEPENDS = $(if $(filter kmod-%,$(1)),$(2) $(PKG_FASTBOOT_DEPENDS),$(PKG_DEFAULT_DEPENDS) $(filter-out $(PKG_DEFAULT_DEPENDS),$(2)))
    TARGET_LDFLAGS += $(if $(filter kmod-%,$(1)),,$(if $(CONFIG_USE_GLIBC),-lrt -lpthread))
  endif
else
  PKG_FIXUP_DEPENDS = $(2)
