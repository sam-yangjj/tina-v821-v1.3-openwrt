...
PREBUILT_SRC:=$(if $(HOST_PREBUILT_SRC),$(HOST_PREBUILT_SRC),$(firstword $(wildcard prebuilt/*)))

ifeq ($(HOST_PREBUILT),1)
ifneq ($(PREBUILT_SRC),)
DL_DIR:=$(CURDIR)
PKG_SOURCE:=$(PREBUILT_SRC)
else
$(error "Please make sure prebuilt package exist")
endif
endif
...
define HostPrebuilt/Prepare
	@echo "HostPrebuilt/Prepare $$@"
ifneq ($(strip $(HOST_UNPACK)),)
	$(HOST_UNPACK)
endif
endef

define HostPrebuilt/Configure
	@echo "HostPrebuilt/Configure $$@"
endef

define HostPrebuilt/Compile
	@echo "HostPrebuilt/Compile $$@"
	$(TOPDIR)/scripts/hostprebuilt_install.sh $(HOST_BUILD_DIR)
endef

define HostPrebuilt/Install
	@echo "HostPrebuilt/Install $$@"
	$(CP) $(HOST_BUILD_DIR)/* $(STAGING_DIR_HOST)/
endef

define HostPrebuilt/Clean
	@echo "HostPrebuilt/Clean $$@"
endef
...
ifndef DUMP
  ...
  ifneq ($(HOST_PREBUILT),1)
	...
	@-rm -rf $(HOST_BUILD_DIR)
	@mkdir -p $(HOST_BUILD_DIR)
	$(call HostPrebuilt/Prepare)
	touch $$@

    $(call Host/Exports,$(HOST_STAMP_CONFIGURED))
    $(HOST_STAMP_CONFIGURED): $(HOST_STAMP_PREPARED)
	$(call HostPrebuilt/Configure)
	touch $$@

    $(call Host/Exports,$(HOST_STAMP_BUILT))
    $(HOST_STAMP_BUILT): $(HOST_STAMP_CONFIGURED)
	$(call HostPrebuilt/Compile)
	touch $$@

    $(HOST_STAMP_INSTALLED): $(HOST_STAMP_BUILT) $(if $(FORCE_HOST_INSTALL),FORCE)
	$(call HostPrebuilt/Install)
	mkdir -p $$(shell dirname $$@)
	touch $(HOST_STAMP_BUILT)
	touch $$@
  endif
