...
AW_BUILD_HOOK_DIR:=$(if $(wildcard $(LICHEE_TOP_DIR)/target/$(LICHEE_IC)/openwrt/$(LICHEE_IC)-common/build_hooks/),$(LICHEE_TOP_DIR)/target/$(LICHEE_IC)/openwrt/$(LICHEE_IC)-common/build_hooks/,$(LICHEE_TOP_DIR)/openwrt/target/$(LICHEE_IC)/$(LICHEE_IC)-common/build_hooks/)

define build_hooks
	if [ -f "$(AW_BUILD_HOOK_DIR)/hook_mkrootfs.sh" ]; then \
		$(AW_BUILD_HOOK_DIR)/hook_mkrootfs.sh $(call mkfs_target_dir,$(target_params)); \
	fi
endef
...
define Image/mkfs/erofs
	$(STAGING_DIR_HOST)/bin/mkfs.erofs -zlz4 \
		$@ $(call mkfs_target_dir,$(1))/
endef
