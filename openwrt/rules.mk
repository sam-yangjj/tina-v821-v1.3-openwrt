...
-include $(LICHEE_TOP_DIR)/target/$(LICHEE_IC)/openwrt/generic/BoardRules_generic.mk
-include $(LICHEE_TOP_DIR)/target/$(LICHEE_IC)/openwrt/$(LICHEE_IC)-common/BoardRules_common.mk
-include $(LICHEE_TOP_DIR)/target/$(LICHEE_IC)/openwrt/$(LICHEE_IC)-$(LICHEE_BOARD)/BoardRules.mk
...
ifndef DUMP
  ifeq ($(CONFIG_EXTERNAL_TOOLCHAIN),)
    ...
    ifeq ($(CONFIG_NATIVE_TOOLCHAIN),)
      ...
      TOOLCHAIN_ROOT_TAR_XZ:=$(patsubst %/,%,$(TOOLCHAIN_ROOT_DIR)).tar.xz
      $(if $(wildcard $(TOOLCHAIN_ROOT_TAR_XZ)),$(if $(wildcard $(TOOLCHAIN_ROOT_DIR)/.$(shell stat -c %Y $(TOOLCHAIN_ROOT_TAR_XZ))_decompressed),,$(info Prepare openwrt toolchain ...)$(shell rm -rf $(TOOLCHAIN_ROOT_DIR) && mkdir -p $(TOOLCHAIN_ROOT_DIR) && tar --strip-components=1 -xf $(TOOLCHAIN_ROOT_TAR_XZ) -C $(TOOLCHAIN_ROOT_DIR) && touch $(TOOLCHAIN_ROOT_DIR)/.$(shell stat -c %Y $(TOOLCHAIN_ROOT_TAR_XZ))_decompressed)),)
